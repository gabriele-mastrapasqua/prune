...
impl DrawData<'_> {
    ...
    fn generate_bar(&self, node: &DisplayNode, level: usize) -> String {
        ...
        let use_gradient = self.display_data.initial.colors_on;

        let chars_in_bar = self.percent_bar.chars().count();
        let percent = self.display_data.percent_size(node);
        let num_bars = chars_in_bar as f32 * percent;
        ...
        let mut filled_chars = Vec::new();
        for c in itr {
            num_not_my_bar -= 1;
            if num_not_my_bar <= 0 {
                filled_chars.push(BLOCKS[0]);
            } else if c == BLOCKS[0] {
                filled_chars.push(BLOCKS[idx]);
            } else {
                filled_chars.push(c);
            }
        }
        ...
        if !self.display_data.initial.bars_on_right {
            filled_chars.reverse();
        }

        if use_gradient {
            // Apply gradient colors: green → yellow → red
            let total_filled = filled_chars.iter().filter(|&&c| c == BLOCKS[0]).count();
            let mut filled_idx = 0;

            for c in filled_chars {
                if c == BLOCKS[0] {
                    // Calculate gradient position for this filled character
                    let gradient_pos = if total_filled > 1 {
                        filled_idx as f32 / (total_filled - 1) as f32
                    } else {
                        0.0
                    };

                    // Interpolate color: green (0%) → yellow (50%) → red (100%)
                    let (r, g, b) = calculate_gradient_color(gradient_pos, percent);
                    let colored_char = nu_ansi_term::Style::new()
                        .fg(nu_ansi_term::Color::Rgb(r, g, b))
                        .paint(c.to_string())
                        .to_string();
                    new_bar.push_str(&colored_char);
                    filled_idx += 1;
                } else {
                    new_bar.push(c);
                }
...
// Calculate gradient color based on position within bar and overall percentage
fn calculate_gradient_color(position: f32, percentage: f32) -> (u8, u8, u8) {
    // Use the overall percentage to determine the color scheme
    // Low percentage (< 30%): green shades
    // Medium percentage (30-70%): yellow/orange shades
    // High percentage (> 70%): red shades

    if percentage < 0.3 {
        // Green gradient: darker to brighter green
        let intensity = 100 + (position * 155.0) as u8;
        (0, intensity, 0)
    } else if percentage < 0.7 {
        // Yellow to orange gradient
        let r = 255;
        let g = (255.0 - (position * 100.0)) as u8;
        (r, g.max(100), 0)
    } else {
        // Orange to red gradient
        let r = 255;
        let g = (150.0 - (position * 150.0)) as u8;
        (r, g, 0)
    }
}
