...
fn main() {
    ...
    if options.interactive {
        ...
        if let Err(e) = tui::run(dev_mode, min_size, risk) {
    ...
    // Show scan history
    if options.history {
        let history = advisor::history::load_history();
        println!("{}", advisor::history::format_sparkline(&history));
        return;
    }

    // Advisor mode: run advisor and exit
    if options.advisor {
        run_advisor(&options);
        return;
    }
...
fn parse_size(s: &str) -> Option<u64> {
    ...
    let (num_part, multiplier) = if s.ends_with("TB") || s.ends_with("T") {
        ...
        (s.trim_end_matches(|c: char| c.is_alphabetic()), 1u64)
    } else {
        (s.as_str(), 1u64)
    };
...
fn parse_category(s: &str) -> Option<Category> {
    use advisor::models::DevKind;
    match s.to_lowercase().as_str() {
        "cache" => Some(Category::Cache),
        "log" | "logs" => Some(Category::Log),
        "dev" => Some(Category::Dev(DevKind::BuildArtifact)),
        "ai" => Some(Category::AiModel),
        "installer" | "installers" => Some(Category::Installer),
        "user" | "old-files" => Some(Category::UserOldFiles),
        "system" | "temp" => Some(Category::SystemTemp),
        "docker" => Some(Category::Dev(DevKind::Docker)),
        "simulator" => Some(Category::Dev(DevKind::Simulator)),
        "xcode" => Some(Category::Dev(DevKind::Xcode)),
        _ => None,
    }
}

fn run_advisor(options: &Cli) {
    ...
    let older_than = options.params.as_ref().and_then(|params| {
        for p in params {
            if p.ends_with('d') {
                if let Ok(days) = p.trim_end_matches('d').parse::<u64>() {
                    return Some(days);
                }
            }
        }
    ...
    let engine = AdvisorEngine {
        ...
        risk_limit,
        categories,
        no_dev: options.no_dev,
        no_ai: options.no_ai,
        older_than_days: older_than,
    };

    let recommendations = engine.scan_home();
        ...
        || options.format.as_deref() == Some("json")
    {
        ...
        report::format_report(&recommendations)
    };
