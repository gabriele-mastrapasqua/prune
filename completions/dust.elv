...
set edit:completion:arg-completer[dust] = {|@words|
    ...
    var completions = [
        &'dust'= {
            ...
            cand --dup-min-size 'Minimum size for duplicate scan (e.g. 10M)'
            cand --risk 'Only show recommendations up to this risk level (safe, low, medium, review)'
            cand --format 'Advisor format: table, json, plain'
            cand --category 'Only show recommendations for this category'
            ...
            cand -a 'Run the disk cleanup advisor (macOS only)'
            cand --advisor 'Run the disk cleanup advisor (macOS only)'
            cand --json 'Shorthand for --format json in advisor mode'
            cand -i 'Launch interactive TUI mode'
            cand --interactive 'Launch interactive TUI mode'
            cand --history 'Show scan history sparkline'
            cand --duplicates 'Scan for duplicate files and show recommendations'
            cand --no-dev 'Exclude dev categories from advisor scan'
            cand --no-ai 'Exclude AI categories from advisor scan'
